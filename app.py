...
# --- QUẢN LÝ TRẠNG THÁI (BÊ NGUYÊN TỪ FILE GỐC) ---
# Biến từ bài tập vai.py
counters = {'LEFT': 0, 'RIGHT': 0}
stages = {'LEFT': None, 'RIGHT': None} 

# Biến từ bài tập bàn tay.py
hand_data = {
    "Left":  {"counter": 0, "stage": None, "color": (255, 255, 255)},
    "Right": {"counter": 0, "stage": None, "color": (255, 255, 255)}
}

# Biến từ bài tập khuỷu tay.py
elbow_counters = {'LEFT': 0, 'RIGHT': 0} # bộ đếm số lần
elbow_stages = {'LEFT': None, 'RIGHT': None} #trạng thái chuyển động
    ...
    with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
        while True:
            ...
            if results.pose_landmarks:
                ...
                for side, ids in [('LEFT', [23,11,13,15]), ('RIGHT', [24,12,14,16])]:
                    try:
                        hip, sho, elb, wri = [[landmarks[i].x, landmarks[i].y] for i in ids]
                        angle = calculate_angle(hip, sho, elb)
                        eval_t, eval_c = get_shoulder_eval(angle, side)
                        # Vẽ xương vai
                        ...
                        cv2.putText(frame, f"REPS: {counters[side]}", (10, pos_y+75), 1, 1.8, (0,255,255), 3)
                    except: pass
            ...
            if results.pose_landmarks:
                ...
                for side, ids in [('LEFT', [11, 13, 15]), ('RIGHT', [12, 14, 16])]:
                    try:
                        ...
                        if angle < 50: 
                            elbow_stages[side] = "up"
                        
                        if angle > 150 and elbow_stages[side] == "up":
                            elbow_stages[side] = "down"
                            elbow_counters[side] += 1
                        ...
                        cv2.putText(frame, f"REPS: {elbow_counters[side]}", (10, pos_y+65), 1, 1.3, (0,255,255), 2)
                    except: pass
            ret, buffer = cv2.imencode('.jpg', frame)
            yield (b'--frame\r\n' b'Content-Type: image/jpeg\r\n\r\n' + buffer.tobytes() + b'\r\n')
    cap.release()
